...
echo "$($PSQL "SELECT t.name FROM games AS g INNER JOIN teams AS t ON t.team_id=g.winner_id WHERE g.year=2018 AND round='Final'")"
...
echo "$($PSQL "SELECT g.year, t.name FROM games AS g INNER JOIN teams AS t ON t.team_id=g.winner_id WHERE round='Final' ORDER BY g.year")"
